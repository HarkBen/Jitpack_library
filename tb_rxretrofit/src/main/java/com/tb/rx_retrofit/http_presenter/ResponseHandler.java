...
final public class ResponseHandler extends Subscriber<Response<String>> {
    ...
    /**
     * 协议层 及  responseListener.onResponse(response) 操作数据的异常都会被捕捉
     * 所以这里不全是网络错误
     * @param e
     */
    @Override
    public void onError (Throwable e) {
        //此处可拦截到的异常均发生在请求发起以后
        TBHttpLog.printE(TAG, "onError Throwable subClass :" + e.getClass().getSimpleName() + " - errorMsg:"+e.getMessage());
        e.printStackTrace();
        if(e instanceof NullPointerException){
            ...
            responseListener.onFailure(HttpCode.CODE_UNKNOW, "未知错误");
        }
        responseListener.onFinish();
    }
...
//        TBHttpLog.printI(TAG, "finally Response:" + response.body());
//        TBHttpLog.printI(TAG, "network Response:" + response.raw().networkResponse());
//        TBHttpLog.printI(TAG, "cache Response:" + response.raw().cacheResponse());
